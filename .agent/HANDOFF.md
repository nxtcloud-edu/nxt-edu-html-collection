...
## Next safe action
1. Phase 7 구현·운영 복사·재검증이 완료됐다. 레지스트리 포인터와 공개 읽기는 아직 레거시 `games/*`를 사용한다.
2. 다음 Phase 8은 검증 완료 콘텐츠의 새 키 포인터와 새 키 우선·레거시 fallback 읽기 전환이다.
3. 기존 S3 객체 삭제는 Phase 11이며 Phase 8과 함께 수행하지 않는다.
