...
## Next safe action
1. Phase 3은 `feat: 관리자 코호트 운영 현황 추가` 독립 커밋으로 완료됐다.
2. 다음은 Phase 4 코호트 ID additive backfill이다. 먼저 read-only dry-run과 unresolved 0 기준을 구현하며 승인 전 실제 backfill은 수행하지 않는다.
