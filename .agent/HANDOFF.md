...
## Next safe action
1. 관리자 사용자 생성 코호트 행에 이름 `수정` 버튼과 인라인 저장 폼을 구현했다. 기존 v2 PATCH API를 사용하며 기본 코호트는 계속 수정할 수 없다. 로컬 검증은 완료했고 커밋·push·배포는 실행하지 않았다.
2. 사용자 승인 시 해당 변경만 커밋·push하고 Lambda 코드만 배포해 버튼 노출과 편집 폼을 읽기 전용 검증한다. 실제 운영 코호트 이름 변경은 별도 지시 없이는 실행하지 않는다.
3. 2026-08-30 22:13 KST 이후 Phase 11의 완전한 7일 사용량 근거를 수집하고 fallback 은퇴 dry-run만 재실행한다.
4. fallback apply, 기존 S3 객체 삭제, 운영 콘텐츠·피드백·코호트·계정 변경은 각각 별도 승인 전까지 실행하지 않는다.
