...
- Branch: `main`, 홈 탭 교정 `3b96a99` 배포·origin/main push 완료.
- Worktree: 공개 푸터 관리자 진입 링크 복구 구현·로컬 검증 완료, 배포 전.
...
- 관리자 진입 링크 복구 로컬 완료: 공개 홈·코호트 푸터의 낮은 강조도 `관리자` 링크를 `/admin.html`로 연결하고 모바일 정렬·접근성·E2E를 고정.

## Next safe action
1. 관리자 진입 링크 복구를 커밋·push하고 Lambda 코드만 배포한 뒤 운영 푸터 링크와 로그인 화면을 읽기 전용 검증한다.
