...
- Branch: `main`, 관리자 진입 링크 복구 `37cd642` 배포·origin/main push 완료.
- Worktree: 관리자 사용자 생성 코호트 이름 수정 UI 구현·로컬 검증 완료, 미커밋·미배포.
...
- 관리자 코호트 이름 수정 UI 로컬 완료: 사용자 생성 코호트의 `보관`/`활성화` 옆 `수정` 버튼과 행 내부 저장·취소 폼을 기존 v2 PATCH API에 연결. 기본 코호트 보호 유지, 서버 120/120·웹 2/2·전체 E2E 18/18 통과. 운영 배포는 실행하지 않음.

## Next safe action
1. 사용자 승인 시 코호트 이름 수정 UI를 커밋·push하고 Lambda 코드만 배포해 관리자 화면을 읽기 전용 검증한다.
2. 2026-08-30 22:13 KST 이후 7일 사용량 근거를 수집해 fallback dry-run을 재실행한다.
3. Phase 11 관찰은 병행하되 fallback apply와 기존 `games/*` 삭제는 각각 별도 사용자 승인 전까지 실행하지 않는다.
