...
- Branch: `main`, 관리자 코호트 이름 수정 `feabfed` 배포·origin/main push 완료.
- Worktree: 배포 기록 문서 갱신 외 제품 변경 없음.
...
- 홈 탭 의도 교정 배포 완료: `대시보드`를 첫 탭·기본 화면으로 두고 `콘텐츠 둘러보기`, `수업별 모아보기` 순으로 재구성. 상단·좌측 탐색 명칭도 일치시키고 운영 283·101/182·15, 콘텐츠 첫 10개, 코호트 15개, 오버플로 0 확인.
- 관리자 진입 링크 복구 배포 완료: 공개 홈·코호트 푸터의 낮은 강조도 `관리자` 링크를 `/admin.html`로 연결하고 모바일 정렬·접근성·E2E를 고정. 운영 링크 노출·가로 오버플로 0·관리자 로그인 화면 이동과 Terraform 최종 no changes 확인.
- 관리자 코호트 이름 수정 UI 배포 완료: 사용자 생성 코호트의 `보관`/`활성화` 옆 `수정` 버튼과 행 내부 저장·취소 폼을 기존 v2 PATCH API에 연결. 기본 코호트 보호 유지, 서버 120/120·웹 2/2·전체 E2E 18/18 통과. 운영 사용자 생성 코호트 11개에서 버튼·폼·오버플로 0을 확인했고 실제 이름 저장은 실행하지 않음.

## Next safe action
1. 2026-08-30 22:13 KST 이후 7일 사용량 근거를 수집해 fallback dry-run을 재실행한다.
2. Phase 11 관찰은 병행하되 fallback apply와 기존 `games/*` 삭제는 각각 별도 사용자 승인 전까지 실행하지 않는다.
