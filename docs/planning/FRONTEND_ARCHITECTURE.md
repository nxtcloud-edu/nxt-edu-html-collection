...
- 홈은 운영 지표·콘텐츠 목록·수업 목록을 각각 `대시보드`, `콘텐츠 둘러보기`, `수업별 모아보기` 탭으로 분리한다. 기본 탭은 대시보드이며 기존 `#overview`, `#gallery`, `#cohorts` 해시는 직접 진입과 상단·좌측 탐색 계약으로 유지한다.
...
- 코호트 목록은 불변 `cohortId`를 유지한 채 사용자 생성 코호트의 표시 이름을 행 내부에서 수정하며, 수정 버튼은 보관/활성화 버튼과 같은 작업 영역에 둔다. 기본 코호트는 수정할 수 없다.
