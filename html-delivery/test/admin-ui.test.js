...
test('관리자 HTML 스크립트는 렌더링에 innerHTML을 쓰지 않고 수정 저장은 submit 버튼이다', async () => {
  ...
  assert.match(admin, /<dialog id="cohortRenameModal"/);
  assert.match(admin, /id="openCohortRenameButton"/);
  assert.match(admin, /id="cohortRenameSelect"/);
  assert.match(admin, /id="cohortRenameName"[^>]*maxlength="60"/);
  assert.match(admin, /openCohortRenameButton\.addEventListener\('click',[\s\S]*cohortRenameModal\.showModal\(\)/);
  assert.match(admin, /cohortRenameForm\.addEventListener\('submit',[\s\S]*method:'PATCH'/);
